...
class ExampleUnitTest {
    ...
    fun add_and_remove_from_cart() {
        ...
        runBlockingTest {
            feature.accept(DishesFeature.Msg.AddToCard("test", "test"))
            feature.accept(DishesFeature.Msg.AddToCard("test2", "test2"))
            feature.accept(DishesFeature.Msg.RemoveFromCard("test2", "test2"))
            feature.accept(DishesFeature.Msg.RemoveFromCard("test", "test"))
        }
        ...
        coVerify(ordering = Ordering.SEQUENCE) {
            ...
            mockChannel.send(
                Eff.Notification.Action(
                    ...
                    "Отмена",
                    Msg.Dishes(DishesFeature.Msg.RemoveFromCard("test2", "test2"))
                )
            )
